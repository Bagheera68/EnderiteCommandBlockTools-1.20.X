...
public class ItemParticleEffectsCmdParticle {
            ...
            if (!mc.isPaused() && mc.level != null)
            {
                ...
                for (Entity entity : world.entitiesForRendering())
                {
                    if (entity instanceof ItemEntity)
                    {
                        ...
                        if (item.getItem().is(ModItems.Enderite_Cmd_Half_tool.get()) || (item.getItem().is(ModItems.Netherite_Cmd_Upgrade.get())
                                || (item.getItem().is(ModItems.Enderite_Cmd_Upgrade.get()) || item.getItem().is(WitherStormModItemTags.COMMAND_BLOCK_TOOLS))))
                        {
                            for (int i = 0; i < 2; i++)
                            {
